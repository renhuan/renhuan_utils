...
package com.renhuan.utils.tablayout;
...
public class TabLayout extends HorizontalScrollView {
    ...
    private Boolean isOnlySelectShowIcon;

    /**
     * 给tablayout前面设置小图片 在setupWithViewPager之前调用
     *
     * @param mIcons
     * @param isOnlySelectShowIcon true 只有选中的时候有图片,未选中隐藏   false 所有都显示
     */
    public void setIcons(List<Integer> mIcons, boolean isOnlySelectShowIcon) {
        this.mIcons = mIcons;
        this.isOnlySelectShowIcon = isOnlySelectShowIcon;
    }
    ...
    void populateFromPagerAdapter() {
        ...
        if (mPagerAdapter != null) {
            ...
            for (int i = 0; i < adapterCount; i++) {
                //如果有图片,把图片也添加进去
                if (mIcons != null) {
                    addTab(newTab().setText(mPagerAdapter.getPageTitle(i)).setIcon(mIcons.get(i)), false);
                } else {
    ...
    void selectTab(final Tab tab, boolean updateIndicator) {
        ...
        if (currentTab == tab) {
            ...
            if (tab != null) {
                ...
                //如果设置了图片
                if (mIcons != null) {
                    if (isOnlySelectShowIcon) {
                        //只有选中的tab显示图片,并且有个缩放和渐变动画
                        for (int i = 0; i < mTabStrip.getChildCount(); i++) {
                            TabView tabView = (TabView) mTabStrip.getChildAt(i);
                            if (tabView.getTab() == tab) {
                                tabView.mIconView.setVisibility(View.VISIBLE);
                            } else {
                                tabView.mIconView.setVisibility(View.GONE);
                            }
                        }
                    }
                    //添加图片隐藏动画
                    ...
                    animatorSet.playTogether(
                            ObjectAnimator.ofFloat(tab.mView.mIconView, "scaleX", 0, 1),
                            ObjectAnimator.ofFloat(tab.mView.mIconView, "scaleY", 0, 1),
                            ObjectAnimator.ofFloat(tab.mView.mIconView, "alpha", 0, 1)
                    );
                    animatorSet.start();
                }
            }
...
//                            .inflate(R.layout.design_layout_tab_text, this, false);
    ...
    public class SlidingTabStrip extends LinearLayout {
        ...
        //tab指示条圆角
        private float tabRoundCorners = dpToPx(30);
        ...
        protected void dispatchDraw(Canvas canvas) {
            canvas.drawRoundRect(left, (getHeight() - mSelectedIndicatorHeight) / 2f,
                    right, getHeight() / 2 + mSelectedIndicatorHeight / 2f, tabRoundCorners, tabRoundCorners, mSelectedIndicatorPaint);
